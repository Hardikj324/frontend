...
export default function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!pv.isValid) { setError(pv.message); return; }
  ...
  const Field = ({ icon:Icon, label, type, value, onChange, placeholder, extra='' }) => (
    <div>
  ...
  return (
    <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}
      ...
      <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.1rem' }}>
        ...
        <Field icon={FiLock} label="Confirm Password" type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Repeat password" />
